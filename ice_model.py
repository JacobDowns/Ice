...
class IceModel(object):
  ...
  def run_model(self, dt, t_end):
    ...
    while t < t_end:
        ...
        coupled_problem = NonlinearVariationalProblem(self.R, self.U, bcs=[self.dbc0, self.dbc1, self.dbc3], J=self.J)
        coupled_problem.set_bounds(self.l_bound, self.u_bound)
        coupled_solver = NonlinearVariationalSolver(coupled_problem)
        # Accquire the optimizations in fenics_optimizations
        set_solver_options(coupled_solver)
